...
import { Form, Input, Textarea, DatePicker, RadioGroup, Radio, CheckboxGroup, Checkbox } from "@heroui/react";
...
async function taskAction(prevState: TaskFormState, formData: FormData): Promise<TaskFormState> {
  // Existing task logic
...
const AddTask: React.FC = () => {
  ...
  return (
    <section className="mt-6 p-6 border border-zinc-700 rounded-md">
      {state.message && (
        <div className={cn(state.isError ? 'bg-red-800' : 'bg-green-800',
    ...
    <RadioGroup label="Set Status" name="status" defaultValue={TaskStatus.open}>
    ...
    <DatePicker
      ...
      name="deadline"
      value={valueDueDate}
      onChange={(valueDueDate) => valueDueDate && setValueDueDate(valueDueDate)}
      size="sm"
      variant="faded"
    />
    <p className="text-default-500 text-sm">
          Selected date: {valueDueDate ? formatter.format(valueDueDate.toDate(getLocalTimeZone())) : "--"}
        </p>

    <RadioGroup label="Set Priority" name="priority">
      {Object.values(TaskPriority).map((priority: string) => (
        <Radio key={priority} value={priority}>{priority}</Radio>
      ))}
    </RadioGroup>
    ...
    <RadioGroup label="Set Assignee" name="assignee" defaultValue={"John Snow"}>
    ...
    <CheckboxGroup label="Set Dependencies" name="dependencies" defaultValue={["task-1", "task-2"]}>
